...
Eleni i ostali u svom radu navode kako se pojam Chatterbot prvi put spominje 1991. godine, i to kao umjetni “igrač” u TINYMUD-u (\textit{multiplayer real-time virtual world}), čija je primarna funkcija bila razgovor. Autori navode kako su mnogi stvarni (ljudski) igrači čak više voljeli razgovarati s Chatterbotom nego s drugim stvarnim igračem \cite{adamopoulou2020chatbots}.
...
\begin{itemize}
  \item Transformer arhitektura omogućava paralelno treniranje, čime se olakšava skaliranje na vrlo velike modele.
  \item Ključna prednost je sposobnost modeliranja odnosa među elementima ulaza i u dugim ili složenim rečenicama.
\end{itemize}

\subsection{\emph{Attention} mehanizam}

\begin{itemize}
  \item \emph{Attention} opisuje način na koji model procjenjuje koji su dijelovi ulaza relevantni za generiranje izlaza.
  \item Višeglavi (\emph{multi-head}) \emph{attention} omogućuje istovremeno fokusiranje na različite aspekte značenja (npr. referenca zamjenica, semantičke veze, stil).
\end{itemize}

\section{Rad s LLM-ovima u aplikacijama: tokeni, kontekst i kontrola generiranja}
\label{sec:tokeni-kontekst-generiranje}
\subsection{Tokeni i tokenizacija}
\label{subsec:tokeni}
\begin{itemize}
  \item Token predstavlja osnovnu jedinicu teksta koju model obrađuje (ponekad cijela riječ, ponekad dio riječi ili interpunkcija).
  \item Model generira izlaz sekvencijalno, token po token, što izravno utječe na cijenu i vrijeme izvođenja.
\end{itemize}

\subsection{Kontekst modela}
\label{subsec:kontekst}
\begin{itemize}
  \item Kontekst označava maksimalnu količinu teksta s kojom model može raditi odjednom (analogno radnoj memoriji).
  \item Ograničenja konteksta motiviraju tehnike poput sažimanja, \emph{chunking}-a i RAG pristupa.
\end{itemize}

\subsection{Parametri generiranja: \emph{temperature}, top-k i top-p}
\label{subsec:parametri-generiranja}
\begin{itemize}
  \item \textbf{\emph{Temperature}} kontrolira stupanj stohastičnosti: niže vrijednosti daju predvidljivije i stabilnije odgovore, više vrijednosti potiču raznolikost.
  \item \textbf{Top-k} ograničava izbor sljedećeg tokena na $k$ najvjerojatnijih kandidata.
  \item \textbf{Top-p} (\emph{nucleus sampling}) odabire najmanji skup tokena čija kumulativna vjerojatnost prelazi prag $p$.
\end{itemize}

\section{Prilagodba ponašanja bez treniranja: prompt engineering i \emph{in-context} učenje}
\label{sec:prompting-icl}
\subsection{Prompt engineering}
\label{subsec:prompt-engineering}
\begin{itemize}
  \item Prompt engineering predstavlja oblikovanje uputa modelu (npr. zadavanje uloge, očekivanog formata izlaza i pravila) radi konzistentnijih rezultata.
\end{itemize}

\subsection{Sistemski i korisnički prompt}
\label{subsec:sistemski-korisnicki}
\begin{itemize}
  \item Sistemski prompt definira opće ponašanje modela u interakciji.
  \item Korisnički prompt predstavlja konkretan zahtjev u određenom trenutku.
\end{itemize}

\subsection{\emph{In-context} learning}
\label{subsec:icl}
\begin{itemize}
  \item \emph{In-context} učenje postiže se davanjem primjera unutar prompta, čime se model privremeno usmjerava bez trajne promjene parametara.
\end{itemize}

\section{Prilagodba modela treniranjem i optimizacije za primjenu}
\label{sec:finetuning-optimizacije}
\subsection{Fine-tuning i PEFT}
\label{subsec:finetuning-peft}
\begin{itemize}
...
\label{subsec:preference-finetuning}
\begin{itemize}
...
\label{subsec:kvantizacija-destilacija}
\begin{itemize}
...
\label{subsec:embeddings-vektorske-baze}
\begin{itemize}
...
\label{sec:evaluacija-operativa}
\subsection{Inferenca: online i batch}
\label{subsec:inferenca}
\begin{itemize}
...
\label{subsec:benchmarkovi-metrike}
\begin{itemize}
...
\label{sec:zakljucak-ai-inzenjerstvo}
\begin{itemize}
